...
This worker is designed as a plug-in component for an existing broader architecture. To simulate integration with the main system, it implements dummy communication channels using RabbitMQ and Redis. The pipeline features fully automated structure analysis, including FFT-based seasonality detection and stationarity testing to dynamically build the optimal parameter grid for ARIMA.
